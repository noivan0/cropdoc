"""
Idea T: ConvNeXt-Base - RESUME from Ep7 (best=0.9538 at Ep6)
Ep8~10 남은 에포크 실행
"""
...
print(f"Resume from best_val={best_val:.4f}")
...
# 남은 3에포크 (Ep8~10), LR은 코사인 스케줄의 Ep7 이후 값으로
optimizer = torch.optim.AdamW([
    {'params': [p for n,p in model_cnx.named_parameters() if 'classifier.2' not in n], 'lr': 1e-6},
    {'params': model_cnx.classifier[2].parameters(), 'lr': 1e-4},
], weight_decay=1e-4)
...
for epoch in range(3):
    ...
    scheduler.step()
    print(f"ConvNeXt Ep{ep_num}: train={t_ok/t_tot:.4f} val={val_acc:.4f} best={best_val:.4f}", flush=True)
    if val_acc > best_val:
        best_val = val_acc
        torch.save({
...
print(f"\nConvNeXt 최종: best_val={best_val:.4f}")
