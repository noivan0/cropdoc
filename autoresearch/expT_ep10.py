"""
Idea T: ConvNeXt-Base Ep10 final
"""
...
model_cnx = convnext_base(weights=None)
in_features = model_cnx.classifier[2].in_features
model_cnx.classifier[2] = nn.Linear(in_features, NUM_NEW)

ck_t = torch.load('data/models/cropdoc_ext_expT/model.pt', map_location='cpu')
model_cnx.load_state_dict(ck_t['model_state_dict'])
best_val = ck_t['val_acc_new']
model_cnx = model_cnx.to(device)
print(f"Resume best_val={best_val:.4f}")
...
class NewClassDataset(Dataset):
    def __init__(self, tf):
        ...
        for i, lbl in enumerate(sorted_classes):
            ...
            for p in imgs[:600]: self.samples.append((p, i))
...
optimizer = torch.optim.AdamW([
    {'params': [p for n,p in model_cnx.named_parameters() if 'classifier.2' not in n], 'lr': 5e-7},
    {'params': model_cnx.classifier[2].parameters(), 'lr': 5e-5},
], weight_decay=1e-4)
...
print(f"ConvNeXt Ep10: train={t_ok/t_tot:.4f} val={val_acc:.4f} best={best_val:.4f}", flush=True)
if val_acc > best_val:
    best_val = val_acc
    torch.save({
        ...
        'stage': 'T',
    }, 'data/models/cropdoc_ext_expT/model.pt')
    print(f"  → 새 베스트! {best_val:.4f}")

print(f"\nConvNeXt-Base 최종 best_val={best_val:.4f}")
if best_val >= 0.9779:
    print("✅ KEEP 기준 달성!")
else:
    print("❌ KEEP 미달, 앙상블로 시도")
