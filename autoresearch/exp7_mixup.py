"""
Idea 7: MixUp augmentation + 더 미세한 LR
기반: exp6 모델 (val_acc_new=0.9791)
새 baseline = 0.9791, keep 기준: >= 0.9891
전략: MixUp(alpha=0.2) + Differential LR 더 작게 (backbone=5e-7, classifier=2e-6)
"""
...
ck = torch.load('data/models/cropdoc_ext_exp6/model.pt', map_location='cpu')
NUM_OLD = ck['num_old']
NEW_CLASSES = ck['new_classes']
NUM_NEW = len(NEW_CLASSES)
NUM_TOTAL = ck['num_classes']
id2label = ck['id2label']
print(f"exp6 baseline: {ck['val_acc_new']:.4f} → keep 기준: >=0.9891")
...
optimizer = torch.optim.AdamW([
    {'params': backbone_params, 'lr': 5e-7, 'weight_decay': 1e-4},
    {'params': classifier_params, 'lr': 2e-6, 'weight_decay': 1e-4},
])
scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=EPOCHS)

OUT_DIR = "data/models/cropdoc_ext_exp7"
...
for epoch in range(EPOCHS):
    ...
    for imgs, labels in train_loader:
        ...
        # MixUp
        mixed_imgs, ya, yb, lam = mixup_data(imgs, new_labels, alpha=0.2)
        optimizer.zero_grad()
        out = model(mixed_imgs)
        new_out = out[:, NUM_OLD:]
        loss = mixup_criterion(criterion, new_out, ya, yb, lam)
        loss.backward()
        torch.nn.utils.clip_grad_norm_(model.parameters(), 1.0)
        optimizer.step()
        preds = new_out.argmax(1)
        t_ok += (preds == new_labels).sum().item()
        t_tot += len(labels)
...
print(f"\nIdea7 완료: val_acc_new={best_val:.4f}, 소요={total_time:.1f}min")
print(f"keep 기준(>=0.9891): {'KEEP ✓' if best_val >= 0.9891 else 'DISCARD ✗'}")
