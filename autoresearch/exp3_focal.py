"""
Idea 3: Focal Loss (gamma=2.0) + 클래스 가중 샘플링
기반: exp2 모델 (backbone[-2:] unfreeze + 강화 Aug)
새 baseline = 0.9499, keep 기준: >= 0.9599
"""
...
# exp2 모델 로드
ck = torch.load('data/models/cropdoc_ext_exp2/model.pt', map_location='cpu')
NUM_OLD = ck['num_old']
NEW_CLASSES = ck['new_classes']
NUM_NEW = len(NEW_CLASSES)
NUM_TOTAL = ck['num_classes']
id2label = ck['id2label']
print(f"로드 완료: NUM_OLD={NUM_OLD}, NUM_NEW={NUM_NEW}, NUM_TOTAL={NUM_TOTAL}")
print(f"exp2 val_acc_new baseline: {ck['val_acc_new']:.4f}")
...
# features.6, 7 + classifier unfreeze (exp2와 동일)
for param in model.parameters():
    param.requires_grad = False
for name, param in model.named_parameters():
    if any(p in name for p in ['classifier', 'features.6', 'features.7']):
...
print(f"전체 샘플: {len(full_ds)}")
print(f"클래스별 샘플 수: {dict(sorted(full_ds.class_counts.items()))}")
...
OUT_DIR = "data/models/cropdoc_ext_exp3"
...
for epoch in range(8):
    ...
    scheduler.step()
    print(f"Ep{epoch+1}: train={t_ok/t_tot:.4f} val={val_acc:.4f} [{elapsed:.1f}min]")
    if val_acc > best_val:
        best_val = val_acc
        torch.save({
...
print(f"\nIdea3 완료: val_acc_new={best_val:.4f}, 소요={total_time:.1f}min")
print(f"keep 기준(>=0.9599): {'KEEP ✓' if best_val >= 0.9599 else 'DISCARD ✗'}")
