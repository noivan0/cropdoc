"""
Idea 7: Polish fine-tuning — exp3(95.65%) 기반, 매우 낮은 LR로 polish
- features.6,7 + classifier
- LR=5e-6 (매우 낮게)
- 10 epochs, cosine
- Label smoothing=0.05 (낮게)
- 목표: 95.65% → 96.6%+ (threshold 달성)
"""
...
EXP_DIR      = os.path.join(BASE_DIR, "data/models/cropdoc_ext_exp7")
...
# features.6,7 + classifier (narrower = safer)
...
# Train에서 더 aggressive augmentation 없이 minimal로
TRAIN_TF = T.Compose([
    T.Resize(256),
    T.RandomCrop(224),
...
print(f"\n[3/5] Polish fine-tuning ({EPOCHS} epochs, LR={LR})...")
...
print(f"  vs exp3(95.65%): {best_val_acc-0.9565:+.4f}")
...
print(f"\n[RESULT] idea7_polish  val_acc={best_val_acc:.4f}  status={status}")
