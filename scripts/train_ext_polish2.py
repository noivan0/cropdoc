"""
Idea 8: exp7(96.07%) 기반 추가 polish — 더 낮은 LR 지속
- exp7에서 시작
- LR=2e-6, 10 epochs, cosine
- 목표: 96.6%+ (threshold 달성)
"""
...
START_MODEL  = os.path.join(BASE_DIR, "data/models/cropdoc_ext_exp7/model.pt")
EXP_DIR      = os.path.join(BASE_DIR, "data/models/cropdoc_ext_exp8")
...
print(f"  exp7 val_acc_new={ck['val_acc_new']:.4f}")
...
print(f"\n[3/5] Polish2 ({EPOCHS} epochs, LR={LR})...")
...
print(f"  improvement from baseline: {best_val_acc-BASELINE:+.4f}")
print(f"  vs exp7(96.07%): {best_val_acc-0.9607:+.4f}")

if best_val_acc >= THRESHOLD:
    print(f"🎉 KEEP! threshold 달성!")
    import shutil
    shutil.copy(os.path.join(EXP_DIR, "model.pt"),
                os.path.join(BASE_DIR, "data/models/cropdoc_efficientnet_v2_extended/model.pt"))
    status = "keep"
else:
    print(f"❌ DISCARD (threshold: {THRESHOLD:.4f}, gap: {THRESHOLD-best_val_acc:.4f})")
    status = "discard"

print(f"\n[RESULT] idea8_polish2  val_acc={best_val_acc:.4f}  status={status}")
