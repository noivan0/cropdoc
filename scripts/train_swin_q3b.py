"""
Task 1 (cont): Swin V2-S Q3 — ep7~15 이어서, best=0.9892에서 시작
"""
...
print(f"Swin 로드: val_acc={ck['val_acc_new']:.4f}, stage={ck.get('stage')}")
...
# ep7부터 이어서: CosineAnnealing T_0=5, T_mult=2에서 ep6까지 진행했으니
# scheduler.step()을 6번 호출한 것과 동일한 상태에서 시작
...
# ep 1~6 진행한 것처럼 scheduler 상태 맞추기
for _ in range(6):
    scheduler.step()
...
for epoch_idx in range(9):
    ...
    scheduler.step()
    print(f"Swin Ep{ep:02d}: train={t_ok/t_tot:.4f} val={val_acc:.4f} best={best_val:.4f}", flush=True)
    if val_acc > best_val:
        best_val = val_acc
        torch.save({
...
print(f"Swin Q3 최종: {best_val:.4f}")
