...
ck = torch.load('data/models/cropdoc_ext_convnext_large/model.pt', map_location='cpu')
...
print(f"CNX-L 로드: val_acc={ck['val_acc_new']:.4f}, stage={ck.get('stage')}")
...
# ep1~4 진행한 것처럼 scheduler 상태 맞추기
for _ in range(4):
    scheduler.step()
...
for epoch_idx in range(6):
    ...
    scheduler.step()
    print(f"CNX-L Ep{ep:02d}: train={t_ok/t_tot:.4f} val={val_acc:.4f} best={best_val:.4f}", flush=True)
    if val_acc > best_val:
        best_val = val_acc
        torch.save({
