"""
Idea 3 강화: Focal Loss + deeper unfreeze (features.5,6,7 + classifier)
backbone[-3:] unfreeze, LR=2e-5, 12 epoch, Focal Loss (gamma=2, ls=0.1)
"""
...
CURR_MODEL = os.path.join(BASE_DIR, "data/models/cropdoc_ext_exp1/model.pt")
EXP_DIR    = os.path.join(BASE_DIR, "data/models/cropdoc_ext_exp3")
...
# Deeper unfreeze: features.5, features.6, features.7 + classifier
for p in model.parameters():
    p.requires_grad = False
unfreeze_patterns = ['classifier', 'features.5', 'features.6', 'features.7']
...
print(f"  Unfreeze patterns: {unfreeze_patterns}")
...
print(f"\n[3/5] Focal Loss 학습 ({EPOCHS} epochs, deeper unfreeze)...")
...
for epoch in range(EPOCHS):
    ...
    if elapsed > 14.5:  # 15분 제한
        print(f"  ⏱️ Time limit reached at epoch {epoch+1}, stopping early")
        break
...
print(f"\n[RESULT] idea3_focal  val_acc={best_val_acc:.4f}  status={status}  (baseline {BASELINE:.4f})")
